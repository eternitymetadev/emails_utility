...
class EmployeesImport implements ToModel, WithHeadingRow
{
    ...
    public $emails_to_delete = [];
    public $emails_to_create = [];
    public $emails_to_block = [];
    ...
    public function model(array $row)
    {
        ...
        if ($employee) {
            ...
            if ($dateOfLeaving) {
                $daysSinceLastWorkingDate = Carbon::parse($dateOfLeaving)->diffInDays(Carbon::now());
                
                if ($daysSinceLastWorkingDate > 30) {
                    // If it's been more than 30 days, delete the email

                     $this->emails_to_delete[] = $employeeData;
                } else {
                    // If it's been 30 days or less, block the email
                    $this->emails_to_block[] = $employeeData;
                }
            }
            ...
            $employee = Employee::create($employeeData);
            $this->emails_to_create[] = $employeeData;
         }
       }
